...
@Solution(
    id = 1436, title = "旅行终点站", difficulty = Difficulty.EASY, description = """
...
fun destCity(paths: List<List<String>>): String {
    return paths.map { it[1] }.toHashSet().subtract(paths.map { it[0] }).first()
}
...
fun main() {
    ...
    println(destCity(listOf(listOf("B", "C"), listOf("D", "B"), listOf("C", "A"))))
    println(destCity(listOf(listOf("A", "Z"))))
}