...
@Solution(
    id = 748, title = "最短补全词", difficulty = Difficulty.EASY, description = """
...
fun shortestCompletingWord(licensePlate: String, words: Array<String>): String {
    fun countLetters(word: String) = word.lowercase().filter { it.isLetter() }.groupingBy { it }.eachCount()

    val cnt: Map<Char, Int> = countLetters(licensePlate)
    return words.filter { w ->
        val m = countLetters(w)
        cnt.all { m.getOrDefault(it.key, 0) - it.value >= 0 }
